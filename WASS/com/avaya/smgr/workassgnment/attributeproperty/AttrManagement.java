...
public void Verify_Err_AttrField() throws Exception
{
	action.RefreshPage();
	//Navigate to work assignment
	action.ClickLink(locator.getProperty("Work_Assignment"));
	...
	wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(locator.getProperty("StatusMsgbox"))));
	//Verify Message on Successful addition of attribute
	action.VerifyStringValue("Attribute has invalid content.");
	Thread.sleep(1000);
}

@Test(description="Verify the add Attribute operation",priority=3)
...
@Test(description="Verify the error meassage on adding category with same name",priority=4)
...
@Test(description="Verify the error meassage on deleting category before deleing attribute",priority=5)
...
@Test(description="Verify the Deletion of Attribute",priority=6)
...
@Test(description="Verify the Deletion of category",priority=7)
