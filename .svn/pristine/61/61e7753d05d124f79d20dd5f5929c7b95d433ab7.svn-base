...
public class VerifySiteoptions {
...
@Test(description="Verify the Site list options after selecting specific tenant") 
public void verifytenantsitoption() throws Exception
{
	...
	Thread.sleep(2000);
	//Verify the Site options
	WebElement selectElement =action.driver.findElement(By.xpath(locator.getProperty("Users.sitelist")));
	Select select = new Select(selectElement);
	List<WebElement> options=select.getOptions();
	int n=options.size();
	Thread.sleep(2000);
	String[] exp ={"",input.getProperty("site1"),input.getProperty("Site2")};
	for(int i=0;i<n;i++)
	{
